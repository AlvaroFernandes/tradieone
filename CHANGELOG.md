...
- Reproduced and fixed the "An error occurred while saving the entity changes" 400 users hit when editing a client's ABN: the `abn` DB column rejects anything but digits, so typing it with the spaces or dashes people naturally use (`12 345 678 901`) failed the save. Both `use-edit-client.ts` and `use-create-client.ts` now strip non-digit characters before sending.
- The DB CHECK on `abn` also caps it at exactly 11 digits (the real AU ABN format) — confirmed a 12+ digit value throws the same 400 even with no separators. `newClientSchema`/`editClientSchema` now validate this client-side ("ABN must be 11 digits") so a bad length is caught before the request goes out instead of surfacing a raw server error.
